...
// ---------------------------------------------------------------------------------------
// ADIÇÃO 3/4: LÓGICA DO PAINEL HOME (CORREÇÃO DE TAREFAS/CATEGORIAS)
// ---------------------------------------------------------------------------------------

  // 1. LÓGICA PRINCIPAL DO PAINEL HOME
  atualizarPainelHome(): void {
    // Certifica que apenas tarefas que são do tipo 'Tarefa' podem ser processadas
    const todasTarefas: Tarefa[] = this.tarefas as Tarefa[];

    if (!todasTarefas || todasTarefas.length === 0) {
      this.tarefasDoDia = [];
      this.contadoresPainel = { atrasadas: 0, hoje: 0, concluidasHoje: 0 };
      // Destrói gráficos se não houver dados
      if (this.chartCategorias) this.chartCategorias.destroy();
      if (this.chartPrioridades) this.chartPrioridades.destroy();
      this.cdr.detectChanges();
      return;
    }

    const dataAtual = moment();
    const hojeFormatado = dataAtual.format('DD/MM/YYYY');

    // TAREFAS PARA HOJE (Abertas e vencendo hoje)
    this.tarefasDoDia = todasTarefas.filter(t => 
      !t.concluida && t.data && moment(t.data, 'DD/MM/YYYY').isSame(dataAtual, 'day')
    );

    this.contadoresPainel.hoje = this.tarefasDoDia.length;

    // TAREFAS ATRASADAS (Abertas e data anterior a hoje)
    this.contadoresPainel.atrasadas = todasTarefas.filter(t =>
      !t.concluida && t.data && moment(t.data, 'DD/MM/YYYY').isBefore(dataAtual, 'day')
    ).length;

    // CONCLUÍDAS HOJE (Concluídas e dataConclusao é hoje)
    this.contadoresPainel.concluidasHoje = todasTarefas.filter(t =>
      t.concluida && t.dataConclusao === hojeFormatado
    ).length;


    this.zone.runOutsideAngular(() => {
      this.criarGraficoCategorias();
      this.criarGraficoPrioridades();
    });
...
  // 3. CRIAÇÃO DO GRÁFICO DE CATEGORIAS
  private criarGraficoCategorias(): void {
    if (this.chartCategorias) { this.chartCategorias.destroy(); }

    const tarefasAbertas: Tarefa[] = (this.tarefas as Tarefa[]).filter(t => !t.concluida);
    const categoriasMap = tarefasAbertas.reduce((acc, t) => {
      const nome = t.categoria ? t.categoria.nome : 'Sem Categoria';
      // Procura a cor na lista principal, se houver
      const corDefault = this.obterCorCategoria(nome);
      
      acc[nome] = (acc[nome] || { count: 0, cor: corDefault });
      acc[nome].count++;
      return acc;
    }, {} as { [key: string]: { count: number, cor: string } });

    const labels = Object.keys(categoriasMap);
    const data = labels.map(label => categoriasMap[label].count);
    const backgroundColors = labels.map(label => categoriasMap[label].cor);

    const canvas = document.getElementById(this.chartCategoriasId) as HTMLCanvasElement;
    if (!canvas) return;

    this.chartCategorias = new Chart(canvas, {
      type: 'doughnut',
      data: { labels: labels, datasets: [{ data: data, backgroundColor: backgroundColors, borderColor: 'rgba(0, 0, 0, 0.5)' }] },
      options: { responsive: true, plugins: { legend: { position: 'right', labels: { color: this.temaClaro ? '#000000' : '#ffffff' } }, title: { display: false } } }
    });
  }

  // 4. CRIAÇÃO DO GRÁFICO DE PRIORIDADES
  private criarGraficoPrioridades(): void {
    if (this.chartPrioridades) { this.chartPrioridades.destroy(); }

    const tarefasAbertas: Tarefa[] = (this.tarefas as Tarefa[]).filter(t => !t.concluida);
    const prioridadesMap = tarefasAbertas.reduce((acc, t) => {
      acc[t.prioridade] = (acc[t.prioridade] || 0) + 1;
      return acc;
    }, {} as { [key: string]: number });

    const labels: ('Alta' | 'Média' | 'Baixa')[] = ['Alta', 'Média', 'Baixa'];
    const data = labels.map(p => prioridadesMap[p] || 0);
    const backgroundColors = ['#dc3545', '#ffc107', '#17a2b8'];
    
    const canvas = document.getElementById(this.chartPrioridadesId) as HTMLCanvasElement;
    if (!canvas) return;
    
    const textColor = this.temaClaro ? '#000000' : '#ffffff';

    this.chartPrioridades = new Chart(canvas, {
      type: 'bar',
      data: { labels: labels, datasets: [{ label: 'Tarefas Abertas', data: data, backgroundColor: backgroundColors, borderWidth: 1 }] },
...
        categoria: categoriaObj ? { nome: categoriaObj.nome, cor: categoriaObj.cor } : { nome: '', cor: '' }, 
        concluida: false,
...
        this.tarefasFiltradas();
        this.atualizarPainelHome(); // ADIÇÃO FINAL: Atualiza painel após carregar tarefas
        this.cdr.detectChanges();
      });
    });
  }
...
      // Remove a categoria da lista local para que ela desapareça do menu de categorias
...
    if (this.filtroData) {
      const dataFiltro = moment(this.filtroData).format('DD/MM/YYYY');
      
      lista = lista.filter(t => {
        // Compara strings DD/MM/YYYY
